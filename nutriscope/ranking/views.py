...
def my_ranking(request):
    data = (
        NutriScopeData.objects.values(
            "representative_name", "category"
        )  # 그룹핑 기준
        .annotate(
            rank=Max("total_rank")
        )  # 같은 representative_name 안에서 가장 높은 total_rank
...
def get_product_details(request):
    ...
    if not representative_name:
        return JsonResponse(
            {"error": "제품명이 제공되지 않았습니다."}, status=400
        )
    ...
    return JsonResponse(
        {"representative_name": representative_name, "products": products_list}
    )
